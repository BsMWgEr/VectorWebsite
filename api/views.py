...
def build_update_sold_data(request):
    ...
    if request.POST:
        ...
        dict_fname = q_dict.get('first_name')
        dict_phone_number = q_dict.get('phone_number')
        dict_company = q_dict.get('company')
        dict_lname = q_dict.get('last_name')
        ...
        if dict_fname:
            Customer.objects.filter(id=dict_id).update(first_name=dict_fname)
        if dict_lname:
            Customer.objects.filter(id=dict_id).update(last_name=dict_lname)
        if dict_email:
            Customer.objects.filter(id=dict_id).update(email=dict_email)
...
def get_shipping_address(request):
    if request.GET.get('shipping_customer_id'):
        ...
        other_info = CustomerShippingAddress.objects.all().order_by('-id')
